...
const doroStem = document.getElementById('doro-stem');
const eggDoroFly = document.getElementById('egg-doro-fly');
const eggDoroRun = document.getElementById('egg-doro-run');
const doroAudioFly = document.getElementById('doro-audio-fly');
const doroAudioRun = document.getElementById('doro-audio-run');
let doroRunning = false;
let doroTimers = [];

function setDoroFlyStage(stage) {
  if (eggDoroFly) eggDoroFly.className = 'egg-doro-fly' + (stage ? ' stage-' + stage : '');
}

function setDoroRunStage(stage) {
  if (eggDoroRun) eggDoroRun.className = 'egg-doro-run' + (stage ? ' stage-' + stage : '');
}

function clearDoroTimers() {
  doroTimers.forEach(clearTimeout);
  doroTimers = [];
}

if (doroStem && eggDoroFly && eggDoroRun) {
  const doroFlyImg = eggDoroFly.querySelector('img');
  if (doroFlyImg) {
    doroFlyImg.addEventListener('error', () => {
      eggDoroFly.classList.add('is-empty');
    });
  }
  const doroRunImg = eggDoroRun.querySelector('img');
  if (doroRunImg) {
    doroRunImg.addEventListener('error', () => {
      eggDoroRun.classList.add('is-empty');
    });
  }

  doroStem.addEventListener('click', () => {
    ...
    doroTimers.push(setTimeout(() => {
      setDoroFlyStage('go');
      setDoroRunStage('go');
      playSafe(doroAudioFly);
      playSafe(doroAudioRun);
    }, 20));

    doroTimers.push(setTimeout(() => {
      if (doroAudioFly) doroAudioFly.pause();
      if (doroAudioRun) doroAudioRun.pause();
    }, 5000));
